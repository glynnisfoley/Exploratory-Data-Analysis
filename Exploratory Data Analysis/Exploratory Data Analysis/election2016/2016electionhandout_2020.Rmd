...
ct.kpc<-kpca(census.ct[-c(1,2)], scale=TRUE, center=TRUE)
subct.kpc<-kpca(census.subct[-c(1,2)], scale=TRUE, center=TRUE)
...
kct_rotation <-ct.kpc@rotated
kct.pc<-data.frame(kct_rotation[,1:2])
ksubct_rotation <-subct.kpc@rotated
ksubct.pc<-data.frame(kct_rotation[,1:2])
sorted_pc1 = sort(abs(ct_rotation[,1]),decreasing= TRUE)
head(sorted_pc1)
sorted_kpc1 = sort(abs(kct_rotation[,1]),decreasing= TRUE)
head(sorted_kpc1)
...
head(sorted_pc2)
sorted_kpc2 = sort(abs(ksubct_rotation[,1]),decreasing= TRUE)
head(sorted_pc2)
...
folds.tst <- sample(cut(1:nrow(tst.cl), breaks=nfold, labels=FALSE))
```
...
#nfolds and foldid need to be different for train and test
...
cvLasso.tst = cv.glmnet(x=x.tst, y=y.tst, nfolds = nfolds, foldid = folds.tst, alpha=1,family="binomial", lambda = grid)
bestlam.tst<-cvLasso.tst$lambda.min


out.trn=glmnet(x.trn,y.trn,alpha=1,family = "binomial",lambda=grid)
lasso.coef.trn=predict(out.trn,type="coefficients",s=bestlam.trn)[1:20,]
lasso.coef.trn

out.tst=glmnet(x.tst,y.tst,alpha=1,family = "binomial",lambda=grid)
lasso.coef.tst=predict(out.tst,type="coefficients",s=bestlam.tst)[1:20,]
lasso.coef.tst
